...
public class InsererRemuneration implements Runnable {
	...
	public void run() {
		...
		try {
			...
			ProfilRemuneration profilRemuneration1 = this.profilRemunerationRepository.findById(1)
					.orElseThrow(() -> new EntityNotFoundException("Profil non trouvé"));
			Grade grade1 = this.gradeRepository.findById(2).orElseThrow(() -> new EntityNotFoundException("Grade non trouvé"));
